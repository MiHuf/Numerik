...
\end{pmatrix}
\in \KK^{nxn}
\text{ und } x_1,...,x_2 \in \RR \text{ dann gilt }
0 = Ax =
\begin{pmatrix}
...
\newline
\newpage
\item zu zeigen: A hermitisch, strikt diagonaldominant und $a_{11},...,a_{nn} \text{ positiv}\ \Rightarrow A > 0$\\
\newline
Der Beweis ist analog zu b). Nur folgt aus der strikten Diagonaldominanz
$|a_{ii}| < \sum_{j=1, j\neq i}^n |a_{ij}| \Leftrightarrow |a_{ii}| - \sum_{j=1, j\neq i}^n |a_{ij}| < 0 $ folgendes:\\
\newline
$0 < a_{ii}-\sum_{j=1, j\neq i}^n |a_{ij}| \le \lambda \le a_{ii} + \sum_{j=1, j\neq i}^n |a_{ij}| $ \\
\newline
